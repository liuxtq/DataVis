...
const PatternData = {
  ...
  recommendation: {
    async getRecommendations(offset, limit) {
      try {
        const response = await fetch(
          `${API_BASE_URL}/recommendations?offset=${offset}&limit=${limit}`
        );

        if (!response.ok) {
          throw new Error("获取推荐信息失败");
        }
        ...
        return [
          ...
          {
            targetName: "目标A-123",
            pngUrl: "http://192.168.121.4:8091/images/xml1.png",
            msg: null,
          ...
          {
            targetName: "目标B-456",
            pngUrl: "http://192.168.121.4:8091/images/xml2.png",
            msg: null,
...
// 此处根据实际数据库类型进行修改
const dbTypeMap = {
  relational: ["relational", "kt2.images", "kt2.yanxiquyu"],
  iotdb: ["kt2.seq"],
  mongodb: ["kt2.bw"],
...
// API 基础路径
const API_BASE_URL = "http://192.168.121.4:8091";
  ...
  pics: [
    {
      id: 1,
      targetName: "F21",
      tiffUrl: "/images/sample1.jpg",
      pngUrl: "/images/sample1.jpg",
    },
    {
      id: 2,
      targetName: "F22",
...
const api = {
  ...
  searchData: {
    searchData: async (sql) => {
      try {
        const response = await fetch(`${API_BASE_URL}/searchData`, {
          method: "POST",
          headers: {
  ...
  imageSearch: {
    // 通过上传图片搜索
    queryImages: async (targetName) => {
      try {
        const response = await fetch(`${API_BASE_URL}/queryImages`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
        ...
        return mockData.pics;
      }
    },
    // 通过上传图片搜图
    queryImagesByUpload: async (file) => {
      try {
        const formData = new FormData();
        formData.append("image", file);

        const response = await fetch(`${API_BASE_URL}/queryImagesByUpload`, {
          method: "POST",
          body: formData,
        });
    ...
    // 通过图片URL搜索
    searchImageByUrl: async (path) => {
      try {
        const response = await fetch(`${API_BASE_URL}/queryImage?path=${path}`);
  ...
  recommendation: {
    async getRecommendations(offset, limit) {
      try {
        const response = await fetch(
          `${API_BASE_URL}/recommender?offset=${offset}&limit=${limit}`
        );
    ...
    // 保存recommendSetting
    async recommendSetting(recommendation) {
      // console.log(JSON.stringify(recommendation));
      try {
        const response = await fetch(`${API_BASE_URL}/recommendationSetting`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
    ...
    async getRecommendSetting() {
      try {
        const response = await fetch(
          `${API_BASE_URL}/recommendationSetting?clientIp=`
        );
