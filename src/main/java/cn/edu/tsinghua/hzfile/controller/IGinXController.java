...
import cn.edu.tsinghua.hzfile.dto.OutFile;
import cn.edu.tsinghua.hzfile.dto.SearchDataRequest;
import cn.edu.tsinghua.hzfile.dto.SearchDataResponse;
...
public class IGinXController {
    ...
    public SearchDataResponse searchData(@RequestBody SearchDataRequest request) throws SessionException {



        String sql = "%s offset %d limit %d;";
        SessionExecuteSqlResult sqlResult = session.executeSql(String.format(sql,request.getSql(),request.getCurrentPage(),request.getPageSize()));
        String parseErrorMsg = sqlResult.getParseErrorMsg();
        if (parseErrorMsg != null && !parseErrorMsg.isEmpty()) {
            throw new SessionException(sqlResult.getParseErrorMsg());
        }
        SearchDataResponse response = new SearchDataResponse();
        response.setData(sqlResult.getResultInList(
                false, FormatUtils.DEFAULT_TIME_FORMAT, "ns"));

        sql = "select count(*) from (%s);";
        sqlResult = session.executeSql(String.format(sql,request.getSql()));
        ...
        List<List<String>> queryList =
                sqlResult.getResultInList(
                        false, FormatUtils.DEFAULT_TIME_FORMAT, "ns");
        response.setTotal(Integer.parseInt(queryList.get(1).get(0)));
        return response;
    }
